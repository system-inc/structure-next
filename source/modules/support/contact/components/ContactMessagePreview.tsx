...
export function ContactMessagePreview(properties: ContactMessagePreviewProperties) {
    ...
    return (
        <div className="mx-auto mt-12">
            <div className="mx-auto flex size-8 items-center justify-center rounded-full bg-[#16A34A] text-opsis-action-primary-contrast">
                <CheckIcon className="size-5" />
            </div>

            <h2 className="mt-6 text-center text-2xl font-medium">Message delivered.</h2>
            <p className="mt-4 text-center text-balance text-opsis-content-secondary">
                Your message has been received. We will get back to you as soon as possible.
            </p>

            <FormField label="Your Message" className="mt-10">
                <div className="mb-4 flex flex-col items-stretch rounded-medium border border-opsis-border-primary px-6 py-8 transition-colors">
                    <div className="border-b border-opsis-border-primary pb-6 text-sm font-medium text-opsis-content-secondary transition-colors">
                ...
                {properties.attachmentFiles.map((file, index) => {
                    ...
                    return (
                        <div
                            key={index}
                            className={
                                'mb-2 flex h-14 items-center justify-between rounded-medium bg-opsis-background-secondary px-5 py-3 last:mb-0'
                            }
                        >
                            <FileTypeIcon className="mr-4 size-5" />

                            <div className="min-w-0 flex-1">
                                <p className="truncate text-sm font-medium text-opsis-content-primary">{file.name}</p>
                            </div>

                            <div className="flex items-center gap-4 pl-2">
                                <p className="text-sm text-opsis-content-secondary">{formatFileSize(file.size)}</p>
                            </div>
                        </div>
                    );
                })}
